...
9. 如果题目描述已经明确指定漏洞方向，你必须优先只生成该方向的检测思路，不要扩散到无关漏洞
...
def filter_solutions_by_focus(solutions):
    if not getattr(config, "FOCUS_VULNS", []):
        return solutions
    focused = [solution for solution in solutions if solution["vuln"] in config.FOCUS_VULNS]
    return focused
...
def get_solutions(page, key, session_id):
    ...
    focus_text = f"仅测试这些漏洞类型：{','.join(config.FOCUS_VULNS)}" if config.FOCUS_VULNS else "未指定漏洞方向，允许结合题目链接进行全量测试"
    response = chat(prompt.format(CTF_DESC=config.CTF_DESC, FOCUS_TEXT=focus_text), session_id=session_id)
    logger.info(f"{page['name']} 获取漏洞检测思路：{response}")
    # 解析漏洞检测思路
    solutions = parse_solutions(response)
    solutions = filter_solutions_by_focus(solutions)
    return solutions
