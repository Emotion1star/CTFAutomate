...
def guess_path(url):
    ...
    with ThreadPoolExecutor(max_workers=config.MAX_GUESS_PATH_WORKERS) as executor:
        futures = []
        # 提交所有任务到线程池
...
def explore_all_js(page):
    ...
    # 多线程并发处理
    with ThreadPoolExecutor(max_workers=config.MAX_JS_FETCH_WORKERS) as executor:
...
def explore_page(page, key, vuln, session_id):
    ...
    if not page.get("request"):
        initial_result = request.run({"method": "GET", "url": config.CTF_URL})
        initial_history = initial_result.get("history") or []
        initial_page = add_page(initial_history[-1]) if initial_history else None
        if initial_page:
            add_message(
                message.format(
                    url=initial_page.get("response", {}).get("url", config.CTF_URL),
                    response=initial_page.get("response", {}).get("content", ""),
                    key=key,
                    vuln=vuln,
                    js_info=""
                ),
