...
def add_page(new_page, need_save=False):
    ...
    if not isinstance(new_page, dict):
        logger.warning("add_page收到的页面对象不是字典，已跳过")
        return None
    if "request" not in new_page or "response" not in new_page:
        logger.warning(f"add_page收到的页面对象缺少request/response字段，键集合: {list(new_page.keys())}")
        return None
