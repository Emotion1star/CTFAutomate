...
class Task(db.Model):
    ...
    llm_provider = db.Column(db.String(50), nullable=False, default="")
    llm_model = db.Column(db.String(100), nullable=False, default="")
    llm_profile = db.Column(db.Text, nullable=True)
    ...
    @property
    def llm_profile_dict(self):
        if self.llm_profile:
            try:
                return json.loads(self.llm_profile)
            except (json.JSONDecodeError, TypeError):
                return {}
        return {}

    @llm_profile_dict.setter
    def llm_profile_dict(self, value):
        if value is None:
            self.llm_profile = None
        else:
            self.llm_profile = json.dumps(value, ensure_ascii=False)
    ...
    def result_summary_dict(self, value):
        if value is None:
            ...
            self.result_summary = json.dumps(value, ensure_ascii=False)
    ...
    def to_dict(self, include_messages=False):
        result = {
            ...
            'llm_profile': self.llm_profile_dict,
...
class Page(db.Model):
    ...
    def to_dict(self):
        return {
            ...
            'description': self.description,
            'key': self.key,
            'discovered_at': _serialize_datetime(self.discovered_at),
            'task_id': self.task_id
        }
...
class Message(db.Model):
    ...
    def to_dict(self):
        ...
        return {
            ...
            'metadata': metadata,
            'created_at': _serialize_datetime(self.created_at),
            'timestamp': _serialize_datetime(self.created_at)  # 添加timestamp字段
        }
...
class Agent(db.Model):
    ...
    def to_dict(self):
        ...
        if self.last_heartbeat:
            heartbeat_age_seconds = max(0, int((datetime.utcnow() - self.last_heartbeat).total_seconds()))
        ...
        return {
            ...
            'last_heartbeat': _serialize_datetime(self.last_heartbeat),
            'registered_at': _serialize_datetime(self.registered_at),
            'last_seen': _serialize_datetime(self.last_seen),
        ...
        if 'tasks' in inspector.get_table_names():
            ...
            with db.engine.begin() as connection:
                if 'llm_provider' not in columns:
                    connection.exec_driver_sql("ALTER TABLE tasks ADD COLUMN llm_provider VARCHAR(50) DEFAULT ''")
                if 'llm_model' not in columns:
                    connection.exec_driver_sql("ALTER TABLE tasks ADD COLUMN llm_model VARCHAR(100) DEFAULT ''")
                if 'llm_profile' not in columns:
                    connection.exec_driver_sql("ALTER TABLE tasks ADD COLUMN llm_profile TEXT")
                if 'result_summary' not in columns:
                    connection.exec_driver_sql("ALTER TABLE tasks ADD COLUMN result_summary TEXT")
        print("数据库表创建成功！")
